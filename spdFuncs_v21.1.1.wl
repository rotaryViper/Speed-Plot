...
result=spd`Private`spdEquPlotG[
tangent,
"Tangent",
Column@{
Column@{"Equation:",Defer@equ},
Column@{"Gradient:",grad}
},
dom
],
result={}];
...
spd`Private`spdEquPlotG[Unevaluated@equ,message,message2,dom]={},
...
If[Flatten@{equ}=={},
(*If equation is empty, make it return nothing*)
spd`Private`spdEquPlotGInverse[Unevaluated@equ,message,message2,dom]={},
spd`Private`spdEquPlotGInverse[Unevaluated@equ,message,message2,dom]=
...
findTangentInput=0, findTangentBool=False, findTangentG={{}},
findNormalInput=0, findNormalBool=False, findNormalG={{}},
...
spdPlotFaster[spdFindTangent[equList,findTangentInput,dom[[1]]],PlotStyle->{Dashed,spdColourLine}],
...
(*Find the line and then the point, so that the point is on top*)
spdPlotFaster[spdFindNormal[equList,findNormalInput,dom[[1]]],PlotStyle->{Dashed,spdColourLine}],
spd`Private`spdPointsG[Map[spd`Private`spdInverseGradient@#&,Complement[spdFindX[equList,findNormalInput,dom[[1]],rangeLow,rangeHigh],{{}}]],"Find Normal",spdColourLine]
}
],
SynchronousUpdating->False,
...
{Checkbox[Dynamic[findXBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Find X"},
spd`Private`spdInputField[Dynamic[findXInput,SynchronousUpdating->False],Small],
{Checkbox[Dynamic[findYBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Find Y"},
spd`Private`spdInputField[Dynamic[findYInput,SynchronousUpdating->False],Small],
{Checkbox[Dynamic[findGradBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Find Gradient"},
spd`Private`spdInputField[Dynamic[findGradInput,SynchronousUpdating->False],Small],
{Checkbox[Dynamic[findTangentBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Find Tangent at X"},
spd`Private`spdInputField[Dynamic[findTangentInput,SynchronousUpdating->False],Small],
{Checkbox[Dynamic[findNormalBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Find Normal at X"},
spd`Private`spdInputField[Dynamic[findNormalInput,SynchronousUpdating->False],Small],
(*Checkbox for points of intersection*)
{Checkbox[Dynamic[poiBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Points of Intersect"},
{Checkbox[Dynamic[epBool,SynchronousUpdating->False],Appearance->spdPlotSize],"End Points"},
{Checkbox[Dynamic[holeBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Holes"},
(*Checkbox for toggling asymptotes*)
{Checkbox[Dynamic[asyBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Asymptotes"},
{Checkbox[Dynamic[inverseBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Inverse"},
{Checkbox[Dynamic[derivativeBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Derivative"},
{Checkbox[Dynamic[integrateBool,SynchronousUpdating->False],Appearance->spdPlotSize],"Integrate"}
}], TableSpacing->{0, 0}
],
EventHandler[
Dynamic[
...
Column@{
	Row@{
	name,Checkbox[Dynamic[showAll,SynchronousUpdating->False],Appearance->spdPlotSize]
	},
...
(**)
(*Goals for v21.1*)
(*Fix spdEquPlotG memoization	Done*)
(*Fix tangent and normal finding when closer to 0	Done*)
(**)
(*Goals for v21.1.1*)
(*Correct find normal name for the dot	Done*)
(*Correct find normal gradient for the dot	Done*)
(*Make all checkboxes scale with spdPlotSize	Done? I can't test it cause this feature is broken on linux*)
